...
DB.prototype.insert = function (table, entry, callback) {
	var queryOptions = getQueryOptions(entry);
	
	connection.query("INSERT INTO `" + table + "` (" + queryOptions.fields.join(', ') + ") VALUES (" + queryOptions.placeholder + ")", queryOptions.values, function (err, rows, fields) {
		if (err) {
			console.log(entry);
...
DB.prototype.select = function (table, where, callback) {
	var queryOptions = getQueryOptions(where);
	var checks = [];
	
	for (var property in queryOptions.fields) {
		checks.push('`' + queryOptions.fields[property] + '` = ?');
	}
	
	connection.query("SELECT * FROM `" + table + "` WHERE " + checks.join(' AND '), queryOptions.values, function (err, rows, fields) {
		if (err) {
